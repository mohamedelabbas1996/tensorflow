...
absl::Status ShapeVerifier::CheckAsyncOpComputationShapes(
  ...
  DCHECK(async_opcode == HloOpcode::kAsyncStart ||
         ...
         async_opcode == HloOpcode::kAsyncDone);
  ...
  if (!ShapesSame(async_shape.tuple_shapes(0), param_shape)) {
    const Shape& actual_param_shape = async_shape.tuple_shapes(0);

    if (!actual_param_shape.IsTuple()) {
      return Internal(
          "The %s expects the async shape at index {0} to be a tuple, found "
          "%s.",
          HloOpcodeString(async_op->opcode()),
          actual_param_shape.ToString(/*print_layout=*/true));
    }

    bool is_prefix = false;
    if (actual_param_shape.tuple_shapes().size() <=
        param_shape.tuple_shapes().size()) {
      is_prefix = true;
      for (int i = 0; i < actual_param_shape.tuple_shapes().size(); ++i) {
        if (!ShapesSame(actual_param_shape.tuple_shapes(i),
                        param_shape.tuple_shapes(i))) {
          is_prefix = false;
          break;
        }
      }
  ...
  if (!ShapesSame(async_result_shape, computation_shape.result())) {
    // late binding mode
    bool allowed_late_binding = false;
    if (async_result_shape.IsTuple()) {
      if (async_result_shape.tuple_shapes().empty() &&
          (async_op->opcode() == HloOpcode::kAsyncStart ||
           async_op->opcode() == HloOpcode::kAsyncUpdate)) {
        allowed_late_binding = true;
      } else if (computation_shape.result().IsTuple()) {
        if (async_result_shape.tuple_shapes().size() <=
            computation_shape.result().tuple_shapes().size()) {
          allowed_late_binding = true;
          for (int i = 0; i < async_result_shape.tuple_shapes().size(); ++i) {
            if (!ShapesSame(async_result_shape.tuple_shapes(i),
                            computation_shape.result().tuple_shapes(i))) {
              allowed_late_binding = false;
              break;
            }
          }
        }
      } else if (async_result_shape.tuple_shapes().size() == 1 &&
                 ShapesSame(async_result_shape.tuple_shapes(0),
                            computation_shape.result())) {
        allowed_late_binding = true;
      }
    }

    if (!allowed_late_binding) {
      return Internal(
...
absl::Status ShapeVerifier::HandleAsyncUpdate(HloInstruction* async_update) {
  ...
  if (async_update->operand_count() > 1) {
    ...
    for (int i = 0; i < prev_async_op_params_shape.tuple_shapes().size(); ++i) {
      if (!ShapesSame(curr_async_update_params_shape.tuple_shapes(i),
                      prev_async_op_params_shape.tuple_shapes(i))) {
        return Internal(
            "The %s expects parameter %d in its shape to match operand 0's "
            "parameter shape (%s vs %s).",
            HloOpcodeString(async_update->opcode()), i,
            curr_async_update_params_shape.tuple_shapes(i).ToString(true),
            prev_async_op_params_shape.tuple_shapes(i).ToString(true));
      }
    }
...
absl::Status ShapeVerifier::HandleAsyncDone(HloInstruction* async_done) {
  ...
  const Shape& operand_output_shape =
      async_done->operand(0)->shape().tuple_shapes(1);
  bool all_outputs_bound = false;
  if (ShapesSame(operand_output_shape, computation_shape.result())) {
    all_outputs_bound = true;
  } else if (operand_output_shape.IsTuple() &&
             !computation_shape.result().IsTuple()) {
    if (operand_output_shape.tuple_shapes().size() == 1 &&
        ShapesSame(operand_output_shape.tuple_shapes(0),
                   computation_shape.result())) {
      all_outputs_bound = true;
    }
  }
  if (!all_outputs_bound) {
    return Internal(
        "async-done expects all output buffers to be bound, but found %s vs %s",
        operand_output_shape.ToString(true),
        computation_shape.result().ToString(true));
  }
  ...
  TF_RET_CHECK(current->opcode() == HloOpcode::kAsyncStart);
  chain.push_back(current);
