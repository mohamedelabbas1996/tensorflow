...
bool HloDataflowAnalysis::UpdateAsyncStartValueSet(
  ...
  ShapeUtil::ForEachSubshape(
      root->shape(), [&](const Shape& subshape, const ShapeIndex& index) {
        ...
        if (res_shape.IsTuple()) {
          if (!index.empty() && index[0] >= res_shape.tuple_shapes().size()) {
            return;
          }
        } else {
          if (!index.empty()) {
        ...
        for (const HloValue* val : root_value_set.values()) {
          changed |= value_set.AddValue(val);
        }
      });
  return changed;
}
...
bool HloDataflowAnalysis::UpdateAsyncUpdateValueSet(
  ...
  const Shape& prev_input_prefix =
      async_update->operand(0)->shape().tuple_shapes(0);
